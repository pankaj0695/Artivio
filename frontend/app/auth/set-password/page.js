...
export default function SetPasswordPage() {
  ...
  const onSubmit = async (e) => {
    ...
    if (password.length < 6) {
      toast.error(t("setPassword.passwordMinLength"));
      return;
    }
    if (password !== confirm) {
    ...
    try {
      ...
      if (e?.code === "auth/requires-recent-login") {
        try {
          ...
          toast.error(reauthErr?.message || t("setPassword.reauthFailed"));
        }
      } else if (e?.code === "auth/credential-already-in-use") {
        toast.error(t("setPassword.passwordAlreadySet"));
      } else {
        toast.error(e?.message || t("setPassword.failed"));
      }
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-10">
      <h1 className="text-xl font-semibold mb-4">{t("setPassword.heading")}</h1>
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <Button type="submit" disabled={submitting} className="w-full">
          {submitting ? t("setPassword.saving") : t("setPassword.save")}
        </Button>
      </form>
    </div>
  );
